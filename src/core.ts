...
export const SetWindowPosition: (y: number) => void = r.SetWindowPosition //Set window position on screen (only PLATFORM_DESKTOP) 
export const SetWindowMonitor: (monitor: number) => void = r.SetWindowMonitor //Set monitor for the current window (fullscreen mode) 
export const SetWindowMinSize: (height: number) => void = r.SetWindowMinSize //Set window minimum dimensions (for FLAG_WINDOW_RESIZABLE) 
export const SetWindowSize: (height: number) => void = r.SetWindowSize //Set window dimensions 
...
export const GetMouseRay: (camera: Camera) => Ray = r.GetMouseRay //Returns a ray trace from mouse position 
export const GetWorldToScreen: (camera: Camera) => Vector2 = r.GetWorldToScreen //Returns the screen space position for a 3d world space position 
...
export const Fade: (alpha: number) => Color = r.Fade //Color fade-in or fade-out, alpha goes from 0.0f to 1.0f 
...
export const IsFileExtension: (ext: string) => boolean = r.IsFileExtension //Check file extension 
...
export const GetDirectoryFiles: (count: number) => number = r.GetDirectoryFiles //Get filenames in a directory path (memory should be freed) 
...
export const StorageSaveValue: (value: number) => void = r.StorageSaveValue //Save integer value to storage file (to defined position) 
...
export const IsGamepadName: (name: string) => boolean = r.IsGamepadName //Check gamepad name (if available) 
export const GetGamepadName: (gamepad: number) => string = r.GetGamepadName //Return gamepad internal name id 
export const IsGamepadButtonPressed: (button: number) => boolean = r.IsGamepadButtonPressed //Detect if a gamepad button has been pressed once 
export const IsGamepadButtonDown: (button: number) => boolean = r.IsGamepadButtonDown //Detect if a gamepad button is being pressed 
export const IsGamepadButtonReleased: (button: number) => boolean = r.IsGamepadButtonReleased //Detect if a gamepad button has been released once 
export const IsGamepadButtonUp: (button: number) => boolean = r.IsGamepadButtonUp //Detect if a gamepad button is NOT being pressed 
...
export const GetGamepadAxisMovement: (axis: number) => number = r.GetGamepadAxisMovement //Return axis movement value for a gamepad axis 
...
export const SetMousePosition: (y: number) => void = r.SetMousePosition //Set mouse position XY 
export const SetMouseOffset: (offsetY: number) => void = r.SetMouseOffset //Set mouse offset 
export const SetMouseScale: (scaleY: number) => void = r.SetMouseScale //Set mouse scaling 
...
export const SetCameraMode: (mode: number) => void = r.SetCameraMode //Set camera mode (multiple camera modes available) 
...
export const SetCameraMoveControls: (downKey: number) => void = r.SetCameraMoveControls //Set camera move controls (1st person and 3rd person cameras) 
